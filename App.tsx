...
import { DB, ChatMessage, Project, Event, Company, ProjectDataRow, Scope, Dependency, ChecklistItem, Activity, FileLink, GalleryImage, GalleryFolder, ProtocolData, ProtocolRevision, ProtocolFolder } from './types';
...
import { ColaboradorTab } from './components/ColaboradorTab';
...
export const App = () => {
    ...
    useEffect(() => {
        ...
        const lockKey = `${activeProject.id}-${activeProject.updatedAt}-${(activeProject as any).gallery?.length || 0}`;
        ...
        if (hasLegacyGallery || !hasFolders) {
            console.log("Migration: Iniciando para", activeProject.id);
            ...
            setDb(prev => {
                ...
                if (hasLegacyGallery) {
                    ...
                    delete newProj.gallery;
                    delete newProj.galleryDescriptions;
                }
    ...
    const handleUpdateProtocol = (field: keyof ProtocolData, value: any) => {
    ...
    const handleAddProtocolFolder = () => {
        ...
        const newFolder = { id: `folder-${Date.now()}`, name: newFolderName.trim(), files: [] };
        setDb(prev => ({
            ...prev,
            projects: prev.projects.map(p => p.id === activeProject.id ? {
                ...p,
                protocolData: {
                    ...ensureFolders(p.protocolData),
                    folders: [...(p.protocolData?.folders || []), newFolder]
                },
                updatedAt: new Date().toISOString()
            } : p)
        }));
    ...
    return (
        <div className={`min-h-screen p-4 md:p-8 flex flex-col items-center font-sans relative pb-32 overflow-x-hidden bg-transparent text-theme-text`}>
            ...
            <div className="flex flex-col gap-10 w-full max-w-[1600px]">
                ...
                {hasProject && (
                    <div className="w-full flex justify-center items-center py-3 no-print sticky top-0 z-50 mb-10">
                        <div className="flex bg-[#0f1115] border border-white/5 rounded-full p-1 shadow-2xl backdrop-blur-xl">
                            {[
                                ...
                                { id: 'viabilidade', label: 'Viabilidade', icon: 'fact_check' },
                                            ...
                                            <>
                                                <div className="flex justify-between items-center mb-6 pb-3 border-b border-theme-divider">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="w-0.5 h-5 rounded-full bg-theme-orange"></div>
                                                        <span className="material-symbols-outlined text-base text-theme-orange">folder_open</span>
                                                        <h3 className="font-square font-black text-xs uppercase tracking-widest text-theme-text">{folder.name}</h3>
                                                    </div>
                ...
                {activeTab === 'files' && hasProject && (
                    <div className="animate-fadeIn w-full flex flex-col gap-8 pb-20">
                        {/* Header and Filter */}
                    ...
                    return (
                        <div className="animate-fadeIn ds-card p-0 bg-theme-card h-[calc(100vh-140px)] flex flex-col overflow-hidden">
                            ...
                            <div className="flex items-center justify-between p-4 border-b border-theme-divider bg-theme-card z-10 gap-4 flex-shrink-0">
                                ...
                                {dataSubTab === 'dados' && (
                                    <button onClick={onAddDataRow} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-theme-divider bg-theme-bg text-theme-textMuted hover:text-theme-text hover:border-theme-text text-[10px] font-black uppercase tracking-widest transition-all shadow-sm">
                                            ...
                                            <tbody className="divide-y divide-theme-divider">
                                                {activeProject.dataRows?.map(row => (
                                                    <tr key={row.id} className="hover:bg-theme-highlight transition-colors group">
                                                        ...
                                                        <td className="p-3">
                                                            <div className="flex items-center gap-2">
                                                                <input className="bg-transparent text-xs font-medium w-full outline-none text-theme-text placeholder:text-theme-textMuted/50" placeholder="..." value={row.location} onChange={(e) => onUpdateDataRow(row.id, 'location', e.target.value)} />
                                                                {row.location && (
                                        ...
                                        {previewLocation ? (
                                            <div className="w-full h-full relative group">
                                                <iframe width="100%" height="100%" frameBorder="0" scrolling="no" marginHeight={0} marginWidth={0}
                                                    src={`https://www.google.com/maps?q=${encodeURIComponent(previewLocation)}&output=embed`}
                                                    title="Google Maps" className="grayscale group-hover:grayscale-0 transition-all duration-500"
                                                ></iframe>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            ...
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <select
                                                    ...
                                                    onChange={e => handleUpdateProtocol('status', e.target.value as any)}
                                                >
                                        ...
                                        <div className="flex gap-3 mb-5 p-3 bg-theme-bg rounded-xl border border-theme-divider">
                                            ...
                                            <div className="flex flex-col gap-2 shrink-0">
                                                <select
                                                    ...
                                                    onChange={e => setNewRevisionStatus(e.target.value as any)}
                                                >
                                                    <option value="stopped">Parado</option>
                                                    <option value="approved">Aprovado</option>
                                                ...
                                                {showNewFolderInput && (
                                                    <div className="flex gap-2 px-6 py-3 bg-purple-500/5 border-b border-theme-divider">
                                                        <input
                                                            ...
                                                            onKeyDown={e => e.key === 'Enter' && handleAddProtocolFolder()}
                                                        />
                    ...
                    activeTab === 'viabilidade' && hasProject && (
                        <div className="animate-fadeIn ds-card p-0 bg-theme-card h-[calc(100vh-140px)] flex flex-col overflow-hidden">
                            <div className="flex justify-between items-center px-6 py-4 border-b border-theme-divider bg-theme-card z-10">
                                <div className="flex items-center gap-2">
                                    <div className="w-0.5 h-5 rounded-full bg-theme-orange"></div>
                                    <span className="material-symbols-outlined text-base text-theme-orange">fact_check</span>
                                    <h3 className="font-square font-black text-xs uppercase tracking-widest text-theme-text">Estudo de Viabilidade</h3>
                                </div>
