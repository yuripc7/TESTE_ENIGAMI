...
const Timeline: React.FC<TimelineProps> = ({ project, isExecuted, zoomLevel, setZoomLevel, onBarClick, onBarContextMenu, onAddDependency, onDeleteEvent, isViewer = false }) => {
    ...
    const timelineYear = useMemo(() => {
        if (!project?.timelineStart) return new Date().getFullYear();
        const parts = project.timelineStart.split('-');
        return parseInt(parts[0], 10) || new Date().getFullYear();
    }, [project?.timelineStart]);

    const todayPos = useMemo(() => {
        const today = new Date();
        if (today.getFullYear() !== timelineYear) return -1;
            ...
            <div className="relative" style={{ width: `${totalWidth}px` }}>
                ...
                <div className="sticky top-0 left-0 z-50 flex bg-theme-bg/95 backdrop-blur-md border-b border-theme-divider shadow-lg">
                    <div className="w-64 shrink-0 border-r border-theme-divider flex items-center justify-center bg-theme-card">
                        <span className="text-[9px] font-square font-bold text-theme-textMuted uppercase tracking-widest">{`FLUXO_${String(timelineYear).slice(-2)}`}</span>
                    </div>
