...
function App() {
  ...
  const postpone = (id) => {
    ...
    const moved = {
      ...task,
      dueDate: dateForDay(task.day, offset + 1),
  ...
  const save = (form) => {
    ...
    else setTasks(ts => [...ts, { ...norm, id: Date.now() }]);
  ...
  const onDropDay = (day) => (e) => {
    e.preventDefault();
    if (dragId.current != null) setTasks(ts => ts.map(x => x.id === dragId.current ? { ...x, day, dueDate: dateForDay(day, offset) } : x));
  ...
  ), [tasks, fProject, fPerson, fDisc, onlyValidate, onlyOverdue]);
  ...
  const columns = useMemo(() => {
    ...
    return DAYS.map(d => ({ key: d, label: d, isToday: isCurrentWeek && d === today, list: filtered.filter(x => x.day === d) }));
  ...
  const filtersActive = fProject !== 'all' || fPerson !== 'all' || fDisc !== 'all' || onlyValidate || onlyOverdue;
  const clearFilters = () => { setFProject('all'); setFPerson('all'); setFDisc('all'); setOnlyValidate(false); setOnlyOverdue(false); };
  ...
  const renderCards = (list) => list.map(task => (
    <TaskCard key={task.id} task={task} groupBy={groupBy} showProjColor={t.projColors} canValidate={canValidate} {...cardEvents} />
  ));
  ...
  return (
    <div>
        ...
        <div className="no-print" style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--gap)' }}>
          ...
          <div className="desktop-only" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {exportMenu && (
                <div className="status-menu" style={{ right: 0, left: 'auto', minWidth: 210 }} onClick={e => e.stopPropagation()}>
          ...
          <div className="board" style={{ '--cols': columns.length }}>
            {columns.map(col => (
              <Column key={col.key} meta={{ label: col.label }} tasks={col.list} accent={col.accent} isToday={col.isToday}
                ...
                onDrop={groupBy === 'day' ? onDropDay(col.key) : undefined}>{renderCards(col.list)}</Column>
            ))}
          </div>
        ) : (
          <div className="stack">
            {columns.map(col => (
              <Band key={col.key} meta={{ label: col.label }} tasks={col.list} accent={col.accent} isToday={col.isToday}
                dropping={dropCol === col.key && groupBy === 'day'}
                onDragOver={groupBy === 'day' ? (e) => { e.preventDefault(); setDropCol(col.key); } : undefined}
                onDrop={groupBy === 'day' ? onDropDay(col.key) : undefined}>{renderCards(col.list)}</Band>
            ))}
          </div>
        )}
      ...
      {/* ===== PRINT DOC ===== */}
      <PrintSheet tasks={printTasks} weekLabel={'SEMANA ' + wl} stats={printStats} pva={printPva} trend={trend} scope={exportScope} coord={coord} />
