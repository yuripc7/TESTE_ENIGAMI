...
// ---- Task card ----
function TaskCard({ task, onStatus, onEdit, onDelete, onValidate, onPostpone, onDragStart, onDragEnd, showProjColor, groupBy, canValidate, span }) {
  const pc = projColor(task.project);
  const prog = taskProgress(task);
  const done = isDone(task);
  // em vista por dia, o atraso/prazo só "valem" no último dia do intervalo
  ...
  const dur = (window.taskDuration ? window.taskDuration(task) : (task.durationDays || 1));
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
            ...
            {task.dueDate && showDeadline && (
              <span className={'due' + (overdue ? ' is-over' : dueToday ? ' is-today' : '')}>
                <Icon name={overdue ? 'event_busy' : 'event'} size={12} />{dur > 1 ? 'Entrega ' : ''}{fmtDue(task.dueDate)}{overdue ? ' · ATRASO' : dueToday ? ' · HOJE' : ''}
              </span>
            )}
            {span && !span.isEnd && (
              <span className="due continues"><Icon name="arrow_forward" size={12} />Continua</span>
            )}
            {task.time && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 10.5, fontWeight: 600, color: 'var(--theme-text-muted)' }}>
